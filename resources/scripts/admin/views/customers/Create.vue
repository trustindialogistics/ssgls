...
const isEdit = computed(() => ['customers.edit', 'consignees.edit'].includes(route.name))

let isLoadingContent = computed(() => customerStore.isFetchingInitialSettings)

const pageTitle = computed(() => {
  if (isConsignee.value) {
    return isEdit.value ? 'Edit Consignee' : 'New Consignee'
  }
  return isEdit.value ? 'Edit Consignor' : 'New Consignor'
})
...
async function submitCustomerData() {
  ...
  if (!isEdit.value) {
    data.type = isConsignee.value ? 'CONSIGNEE' : 'CUSTOMER'
  }
  ...
  router.push(isConsignee.value ? `/admin/consignees/${response.data.data.id}/view` : `/admin/customers/${response.data.data.id}/view`)
}
</script>
